...
void setup() {
  ...
  wrapCommand("Connect Wifi","AT+CWJAP=\"...\",\"...\"");

  wrapCommand("Use single connection mode", "AT+CIPMUX=0");

}

void sendPayload(unsigned long ts, int deg, int pos, String devId) {
  String httpPayload = "GET /?ts=" + String(ts) + "&deg=" + String(deg) + "&pos=" + String(pos) + "&devId=" + devId + " HTTP/1.0";
...
void loop() {
  ...
  int intDeg = (int) potSensorLogic();

  wrapCommand("Connect", "AT+CIPSTART=\"TCP\",\"192.168.0.102\",12345", 3000);
  sendPayload(millis(), intDeg, last_position, "dev1");
...
*/
  ...
  */
  ...
  // uncomment a different version if the receiving app is different.





 //OrigKordalFormatOutput() 
...
//  Serial.print(F(","));
  ...
  */
  ...
  return angle_x;
 /*
 // SebastianNilssonRealTimePlotterFormatOutput
 
  // ref   http://sebastiannilsson.com/en/k/projekt/realtime-plotter/
